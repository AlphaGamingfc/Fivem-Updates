	...
	if distance < 5 then
		if amount >= 1 then
			if status < 100 then
				...
				vRP.notify("Give this food to your pet.")
				...
				vRP.notify("Your pet is not hungry anymore.")
			end
		else
			vRP.notify("~sr~You do not have any more food for your pet!~s~")
		end
	else
		vRP.notify("~r~O Your pet is too far away!~s~")
	end
end)
	...
	if not isInVehicle then
		if IsPedSittingInAnyVehicle(GetPlayerPed(-1)) then
			if distance < 8 then
				...
				vRP.notify({"~r~O Your pet is too far from the car!~s~"})
			end
		else
			vRP.notify({"É You need to be inside the car!"})
		end
	else
		if not IsPedSittingInAnyVehicle(GetPlayerPed(-1)) then
			...
			vRP.notify({"You're still inside a car."})
		end
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if status == 0 then
			...
			DeleteEntity(ped)
			vRP.notify({"O Your pet is dead! Next time remember to give her food."})
			come = 3
			status = "die"
		end
	end
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(0)
		if IsControlPressed(0, Keys['F2']) and (GetGameTimer() - GUI.Time) > 150 then
			OpenAnimal()
...
Citizen.CreateThread(function()
	while true do
		...
		if GetDistanceBetweenCoords(coord, Config.Zones.PetShop.Pos.x,Config.Zones.PetShop.Pos.y,Config.Zones.PetShop.Pos.z, false) < 5 then
			DisplayHelpText("Press [~g~E~s~] to open the pet store.")
